...
public class UtilGrafos {
    ...
    public static void imprimirMatrizBooleana(boolean[][] matriz, Map<Comparable, IVertice> vertices, String titulo) {
        if (vertices != null && matriz.length == vertices.keySet().size()) {

            Comparable[] etiquetas = vertices.keySet().toArray(new Comparable[vertices.size()]);
            int etiquetaMasLarga = stringMasLargo(etiquetas);
            int datoMasLargo = 1; // Solo 1 carácter: "1" o "0"
            int separacionEntreColumnas = 3;

            int largo = Math.max(etiquetaMasLarga, datoMasLargo) + separacionEntreColumnas;

            // Formatear etiquetas con espacio fijo
    ...
    public static TGrafoDirigido InvertirGrafo(TGrafoDirigido grafoOriginal) {
        Collection<IVertice> verticesOriginales = grafoOriginal.getVertices().values();
        Collection<IVertice> nuevosVertices = new ArrayList<>();
        for (IVertice v : verticesOriginales) {
            nuevosVertices.add(new TVertice(v.getEtiqueta()));
        }

        // Crea la lista de aristas invertidas
        Collection<IArista> aristasInvertidas = new ArrayList<>();
        for (IVertice v : verticesOriginales) {
            for (IAdyacencia ady : v.getAdyacentes()) {
                Comparable origen = ady.getDestino().getEtiqueta();  // ← se invierte
        ...
        // Crear el nuevo grafo con  vértices y las  aristas invertidas
        return new TGrafoDirigido(nuevosVertices, aristasInvertidas);
    }
    ...
    public void listarTareas (LinkedList orden){
